...
const PROJECTS = [
  ...
  { icon: "🔗", title: "CascadeTrigger — Pipeline Dependency Orchestrator", description: "A config-driven orchestration system that watches upstream data pipelines and automatically triggers downstream jobs the moment all dependencies succeed. Runs in continuous watch mode with configurable polling, supports multi-upstream dependency resolution, and includes Slack notifications with execution links, Snowflake-based audit logging, and API fallback mechanisms. Pipeline config lives entirely in database tables — adding or removing pipelines requires zero code changes.", tags: ["Python", "Snowflake", "REST APIs", "Slack Webhooks", "Pipeline Orchestration", "Dependency Resolution"] },
];
